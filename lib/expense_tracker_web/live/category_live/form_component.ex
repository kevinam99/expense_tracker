defmodule ExpenseTrackerWeb.CategoryLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:monthly_budget]} type="number" label="Monthly budget" step="any" />
        <:actions>
  ...
  def update(%{category: category} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Expenses.change_category(category))
     end)}
  end

  @impl true
  def handle_event("validate", %{"category" => category_params}, socket) do
    changeset = Expenses.change_category(socket.assigns.category, category_params)
